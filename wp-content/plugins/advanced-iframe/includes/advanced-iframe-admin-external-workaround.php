...
<?php
  ...
  _e('<p>
    ...
    echo renderExternalWorkaroundIcon(true);
    _e(' is shown! You only need to define variables before the script or a configuration file if you need to include the SAME ai_external.js with DIFFERENT configurations. See the <a href="//www.tinywebgallery.com/blog/advanced-iframe/advanced-iframe-pro-demo/advanced-iframe-faq#config" target="_blank">FAQ</a> for more details.
    </div>
...
?>  
    <p>
    <?php _e('Defining the variables before the script has the disadvantage that you need to modify the html of the remote domain for every change and also the code there can get really big. Therefore the recommended way is to use config files which are located on the parent server and loaded before the external_ai.js. Config files are by default placed in the folder "advanced-iframe-custom" in the plugin directory and need to follow this naming convention: ai_external_config_&lt;config_id&gt;.js. This file does contain exactly the variables described before. You have then 3 options to include this file. <ol><li>Include a config file to ai_external.js (recommended way) - see below. </li><li>Include the config file directly before ai_external.js. This makes it easy to use different configurations.</li><li>Add the config_id as parameter to the external_ai.js like this:  ..../ai_external.js?config_id=&lt;config_id&gt;. The config_id can only contain alphanumeric characters, - and _.</li></ol>', 'advanced-iframe') ?>
    </p>
    <p>
...
<?php _e('In this example the config_id "example" is used.', 'advanced-iframe'); ?>
    ...
    <li>
	 <?php _e('a. Include the config file to the external_ai.js', 'advanced-iframe') ?><br />
        ...
        or
        <br />
	  <?php _e('c. Add the parameter ?config_id=example to the external_ai.js', 'advanced-iframe') ?><br />
